...
class TensorboardCallback(BaseCallback):
    """
    Custom callback for plotting additional values. Examples for the
    lift task: on each step, plot the cube height, cube success, distance
    from cube to gripper, and grasp info.
    """
    ...
    def _on_rollout_start(self) -> None:
        # re-initialize all the metrics every time an episode happens
        # each class variable is a 2D array, each subarray is the specific metric
        # for the `i`th environment. for ex, self.grasp_data[0] is the grasp data
        # for the 0th environment over the rollout
        self.eef_to_target_distances = [[] for i in range(self.model.n_envs)]
        self.grasp_data = [[] for i in range(self.model.n_envs)]
        self.cube_success_data = [[] for i in range(self.model.n_envs)]
        self.cube_heights = [[] for i in range(self.model.n_envs)]

    def _on_step(self) -> bool:
        n_envs = self.model.n_envs
        env_infos = self.training_env.env_method("get_env_info")
        # - env_infos is an array of len `n_envs` (should be 4 or 8) of dicts
        # - must get the metrics we care about from the dicts and store in the
        # class variables

        for i in range(n_envs):
            eef_dist_env_i = env_infos[i]['cube_to_gripper_dist']
            self.eef_to_target_distances[i].append(eef_dist_env_i)

            grasp_data_env_i = env_infos[i]['cube_grasped']
            self.grasp_data[i].append(grasp_data_env_i)

            cube_success_env_i = env_infos[i]['cube_success']
            self.cube_success_data[i].append(cube_success_env_i)

            cube_height_env_i = env_infos[i]['cube_height']
            self.cube_heights[i].append(cube_height_env_i)

        return True

    def collect_stats(self, envs_data: list) -> dict:
        """
        - accepts a 2D list as a param, returns a dict having mean, stats, min, and max
        - first, collects the mean, std, min, and max over the data for every environment (each sublist)
        - then, after those stats are collected, average over all environments to have the final
            scalar mean, std, min, and max's
        """

        # just check that the param is a 2D list
        assert(all([isinstance(envs_data[i], list)
                    for i in range(len(envs_data))]))

        stats_dict = {}

        # iterate over every env
        for i in range(len(envs_data)):
            curr_env_data = envs_data[i]

            # collect stats for this environment
            mean = float(np.mean(curr_env_data))
            std = float(np.std(curr_env_data))
            min = float(np.min(curr_env_data))
            max = float(np.max(curr_env_data))

            # add the values to the dict
            stats_dict.update({
                'mean': stats_dict.get("mean", 0) + mean,
                'std': stats_dict.get("std", 0) + std,
                'max': stats_dict.get("max", 0) + max,
                'min': stats_dict.get("min", 0) + min,
            })

        # now that the accumulated stats are in the values for the dict, divide by
        # number of envs to make the stats averaged over all envs
        for key in stats_dict.keys():
            stats_dict[key] /= self.model.n_envs

        return stats_dict

    def _on_rollout_end(self) -> None:
        # log metrics over all the class variables, over all envs

        # for each metric, get the mean, min, max, and std over the rollout
        eef_to_target_stats_dict = self.collect_stats(
            self.eef_to_target_distances)
        grasp_stats_dict = self.collect_stats(self.grasp_data)
        cube_success_stats_dict = self.collect_stats(self.cube_success_data)
        cube_heights_stats_dict = self.collect_stats(self.cube_heights)

        # update tensorboard plots with data from the dictionaries
        self.log_TB_metric_stats(
            "eef_to_target_distance", eef_to_target_stats_dict)
        self.log_TB_metric_stats("cube_grasped", grasp_stats_dict)
        self.log_TB_metric_stats("cube_success", cube_success_stats_dict)
        self.log_TB_metric_stats("cube_height", cube_heights_stats_dict)

    def log_TB_metric_stats(self, metric_name: str, metric_stats: dict):
        # get mean, std, min, and max for given metric name
        values = [
            tf.Summary.Value(
                tag=f'{metric_name}/mean_{metric_name}', simple_value=metric_stats['mean']),
            tf.Summary.Value(
                tag=f'{metric_name}/std_{metric_name}', simple_value=metric_stats['std']),
            tf.Summary.Value(
                tag=f'{metric_name}/min_{metric_name}', simple_value=metric_stats['min']),
            tf.Summary.Value(
                tag=f'{metric_name}/max_{metric_name}', simple_value=metric_stats['max']),
        ]
